...
if grep -q ^auth [default=die] /etc/pam.d/system-auth; then
    echo "Already Done"
else
...
if grep -q ^auth [default=die] /etc/pam.d/password-auth; then
    echo "Already Done"
else
    sed -i '/auth        sufficient    pam_unix.so/a auth        [default=die] pam_faillock.so authfail deny=3 unlock_time=600 fail_interval=900\nauth        required      pam_faillock.so authsucc deny=3 unlock_time=600 fail_interval=900' /etc/pam.d/system-auth
fi
...
if grep -q ^pam_unix.so existing_options /etc/pam.d/system-auth; then
    echo "Already Done"
else
...
if grep -q ^SINGLE /etc/sysconfig/init; then
...
if grep logger /etc/init/control-alt-delete.conf; then
