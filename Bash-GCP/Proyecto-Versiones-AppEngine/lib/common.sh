#!/bin/bash

################################################################################
# lib/common.sh - Funciones Comunes Simples
################################################################################

# Colores
COLOR_RED='\033[0;31m'
COLOR_GREEN='\033[0;32m'
COLOR_YELLOW='\033[1;33m'
COLOR_BLUE='\033[0;34m'
COLOR_NC='\033[0m'

# Setup logs
LOGS_DIR="${SCRIPT_ROOT}/logs"
LOG_FILE="${LOGS_DIR}/app-engine-versions.log"

setup_logs() {
    # Detectar si hay variable TICKET_ID del entorno
    local ticket_id="${TICKET_ID:-}"
    
    # Si hay ticket no vacío, usar la carpeta del ticket
    if [[ -n "$ticket_id" ]]; then
        local ticket_logs_dir="/home/admin/Documents/GNP/Tickets/$ticket_id/logs"
        if [[ -d "$ticket_logs_dir" ]]; then
            LOGS_DIR="$ticket_logs_dir"
            local timestamp
            timestamp=$(date +'%Y%m%d_%H%M%S')
            LOG_FILE="${LOGS_DIR}/app-engine-versions-${ticket_id}-${timestamp}.log"
        fi
    fi
    
    # Crear directorio si no existe
    mkdir -p "${LOGS_DIR}"
    touch "${LOG_FILE}"
}

# Logging
log() {
    local level=$1
    shift
    local message="$*"
    local timestamp
    timestamp=$(date '+%Y-%m-%d %H:%M:%S')
    echo "[${timestamp}] [${level}] ${message}" >> "${LOG_FILE}"
}

# Print funciones
print_info() {
    echo -e "${COLOR_BLUE}ℹ${COLOR_NC} $*"
    log "INFO" "$*"
}

print_success() {
    echo -e "${COLOR_GREEN}✓${COLOR_NC} $*"
    log "SUCCESS" "$*"
}

print_warn() {
    echo -e "${COLOR_YELLOW}⚠${COLOR_NC} $*"
    log "WARN" "$*"
}

print_error() {
    echo -e "${COLOR_RED}✗${COLOR_NC} $*" >&2
    log "ERROR" "$*"
}

# Validar dependencias
check_dependencies() {
    local missing=()
    
    for cmd in gcloud jq; do
        if ! command -v "$cmd" &> /dev/null; then
            missing+=("$cmd")
        fi
    done
    
    if [[ ${#missing[@]} -gt 0 ]]; then
        print_error "Falta instalar: ${missing[*]}"
        print_info "Instala con: sudo apt-get install -y google-cloud-sdk jq"
        return 1
    fi
    
    return 0
}

# Validar autenticación GCP
check_gcloud_auth() {
    if ! gcloud auth list --filter=status:ACTIVE --format='value(account)' | grep -q .; then
        print_error "No autenticado en gcloud"
        print_info "Ejecuta: gcloud auth login"
        return 1
    fi
    return 0
}

# Validar proyecto existe
check_project_exists() {
    local project=$1
    
    if ! gcloud projects describe "$project" &>/dev/null; then
        print_error "Proyecto '$project' no existe"
        return 1
    fi
    return 0
}

# Validar App Engine habilitado
check_appengine_enabled() {
    local project=$1
    
    if ! gcloud app describe --project="$project" &>/dev/null; then
        print_error "App Engine no habilitado en '$project'"
        return 1
    fi
    return 0
}

# Morir
die() {
    print_error "$@"
    exit 1
}

# Mostrar barra de progreso
show_progress() {
    local current=$1
    local total=$2
    local width=30
    
    local percent=$((current * 100 / total))
    local filled=$((percent * width / 100))
    local empty=$((width - filled))
    
    printf "\r["
    printf "%${filled}s" | tr ' ' '='
    printf "%${empty}s" | tr ' ' '-'
    printf "] %d/%d (%d%%)" "$current" "$total" "$percent"
}
