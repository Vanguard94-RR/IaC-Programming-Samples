...
while read name ip cluster region project data; do
  ...
  echo -e "${LGRAY}EXEC: ${NC} ${WHITE} gcloud compute --project=$project security-policies rules create 2147483647 --action=deny --security-policy=cve-canary --description="Default rule, higher priority overrides it" --src-ip-ranges=\* ${WHITE}"
  ...
  ## Modificar regla por defecto
  gcloud compute --project=$project security-policies rules create 2147483647 --action=deny --security-policy=cve-canary --description="Default rule, higher priority overrides it" --src-ip-ranges=\*
  ...
  gcloud beta compute ssl-certificates create wildcarddigicertgnpcommx2023 --project=$project --global --description=wildcarddigicertgnpcommx2023 --certificate=bundle-gnp-com-mx.cer --private-key=key_gnp.com.mx_junio_2022

  gcloud services enable containersecurity.googleapis.com
  IFS=$OLDIFS
  for i in $(cat backend-list.txt); do
