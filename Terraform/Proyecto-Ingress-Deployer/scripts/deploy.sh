...
SSL_POLICY_NAME="${SSL_POLICY_NAME:-sslsecure}"
...
# Working copy — all YAML mutations happen here, committed manifests stay clean.
WORK_DIR="$TICKET_DIR/manifests-work"
...
# Redirect to working copy — never mutate committed files
cp "$INGRESS_YAML" "$WORK_DIR/ingress.yaml"
INGRESS_YAML="$WORK_DIR/ingress.yaml"
COMPANIONS_DIR="$WORK_DIR/companions"
...
_yaml_ingress_name=$(yq 'select(.kind == "Ingress") | .metadata.name // ""' "$INGRESS_YAML" | head -1)
...
if [[ "${CI:-false}" != "true" ]]; then
  ...
  _prompt STATIC_IP_NAME "Static IP name (or 'ephemeral' for dynamic IP)" "${_yaml_static_ip:-}"
  _prompt_action
else
...
STATIC_IP_NAME=$(normalize_static_ip_name "${STATIC_IP_NAME:-}")
if [[ -n "${STATIC_IP_NAME:-}" ]] && [[ "$ACTION" != "destroy" ]]; then
  validate_static_ip "$PROJECT_ID" "$STATIC_IP_NAME"
fi
...
FCEOF
    ...
    _gcs_upload "$_fc_companion" "companions/$(basename "$_fc_companion")"
    ok "FrontendConfig generated: $_fc_name → companions/"
    info "FrontendConfig was auto-generated — consider adding it to the GitLab source YAML"
  else
...
if [[ -z "${CLUSTER_NAME:-}" ]]; then
  ...
  if [[ ${#_clusters[@]} -eq 0 ]]; then
    ...
    _ingress_scan=""
    if [[ -n "${_yaml_ingress_name:-}" ]]; then
      info "Scanning clusters for ingress '${_yaml_ingress_name}'..."
      _ingress_scan=$(detect_ingress_cluster "$PROJECT_ID" "$_yaml_ingress_name")
    fi
    _default_sel=1
    for i in "${!_clusters[@]}"; do
      _cname=$(awk '{print $1}' <<< "${_clusters[$i]}")
      _cloc=$(awk '{print $2}' <<< "${_clusters[$i]}")
      _annotation=""
      if echo "$_ingress_scan" | grep -q "^${_cname} "; then
        _ns_found=$(echo "$_ingress_scan" | grep "^${_cname} " | awk '{print $3}')
        _annotation="  [has ingress: namespace=${_ns_found}]"
        _default_sel=$((i+1))
      fi
      printf "  [%d] %-45s %s%s\n" "$((i+1))" "$_cname" "$_cloc" "$_annotation"
    done
    _sel=""
    while true; do
      read -rp "Cluster number [1-${#_clusters[@]}] (default: ${_default_sel}): " _sel
      _sel="${_sel:-$_default_sel}"
    ...
    _DETECTED_INGRESS_NS=$(echo "$_ingress_scan" | grep "^${CLUSTER_NAME} " | awk '{print $3}' | head -1 || true)
...
_TFVARS_TMP=$(mktemp)
trap 'rm -f "$_TFVARS_TMP"' EXIT INT TERM
cat > "$_TFVARS_TMP" << TFVARS
project_id         = "${PROJECT_ID}"
cluster_name       = "${CLUSTER_NAME}"
cluster_location   = "${CLUSTER_LOCATION:-us-central1}"
namespace          = "${NAMESPACE}"
static_ip_name     = "${STATIC_IP_NAME}"
manifests_work_dir = "${WORK_DIR}"
TFVARS

if diff -q "$_TFVARS_TMP" "$TFVARS" &>/dev/null; then
  ok "tfvars unchanged: $TFVARS"
else
  cp "$_TFVARS_TMP" "$TFVARS"
  ok "tfvars updated: $TFVARS"
fi
rm -f "$_TFVARS_TMP"
...
if [[ -z "${_DETECTED_INGRESS_NS:-}" ]] && [[ -n "${_yaml_ingress_name:-}" ]]; then
  _DETECTED_INGRESS_NS=$(kubectl get ingress "$_yaml_ingress_name" -A \
    --no-headers -o custom-columns='NS:.metadata.namespace' 2>/dev/null | head -1 || true)
fi
validate_namespace "$NAMESPACE" "${_DETECTED_INGRESS_NS:-}"
...
if [[ "$ACTION" != "destroy" ]]; then
  ...
  # Marker stored in GCS — migration skipped on subsequent runs.
  _MIGRATION_MARKER="$TICKET_DIR/.frontendconfig-migration-done"
  _MIGRATION_GCS="${GCS_BUCKET}/ingress/.frontendconfig-migration-done"
  _migration_already_done=false
  if [[ -f "$_MIGRATION_MARKER" ]]; then
    _migration_already_done=true
  elif gsutil stat "$_MIGRATION_GCS" &>/dev/null; then
    _migration_already_done=true
    touch "$_MIGRATION_MARKER"
  fi

  if [[ "$_migration_already_done" == "false" ]]; then
    if terraform state list 2>/dev/null | grep -q "kubernetes_manifest\.frontendconfig\[0\]"; then
      ...
      if [[ -z "$_fc_legacy_name" ]]; then
        error "FrontendConfig companion file missing .metadata.name"
        exit 1
      fi
      ...
      touch "$_MIGRATION_MARKER"
      gsutil cp "$_MIGRATION_MARKER" "$_MIGRATION_GCS" &>/dev/null \
        || warn "GCS migration marker upload failed (continuing)"
      ok "State migration complete — marked in GCS"
    fi
  else
    info "FrontendConfig state migration already applied — skipping"
  fi
fi
...
case "$ACTION" in
  ...
  destroy)
    ...
    cd "$TF_DIR"
    terraform destroy -var-file="$TFVARS" -input=false -no-color \
      | tee -a "$LOG_FILE"
